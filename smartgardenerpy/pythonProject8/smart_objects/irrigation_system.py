...
class IrrigationSystem:
    def __init__(self, client):
        ...
        self.state = "OFF"
        self.irrigation_history = []  # Storico dei programmi di irrigazione
    ...
    def turn_on(self):
        ...
        with print_lock:
            print(f"Irrigation system state: {self.state}, timestamp: {timestamp}")
        try:
            # Creo un messaggio JSON
            json_message = json.dumps({"state": self.state})
            self.last_published_message = json_message
            self.client.publish(MqttConfigurationParameters.IRRIGATION_SYSTEM_TOPIC, json_message)
            with print_lock:
                print(f"Irrigation system message: {json_message}")
            self.irrigation_history.append({"timestamp": timestamp, "state": self.state})
    ...
    def to_json(self):
        return {
            "state": self.state,
            "irrigation_history": self.irrigation_history
        }
